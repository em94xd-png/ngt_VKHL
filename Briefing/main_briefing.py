...
pyautogui.press("space", interval=.01)
arrival = os.path.join(script_config.download_path, arrival_file)
...
pyautogui.press("tab", presses=6, interval=.01)
pyautogui.press("space", interval=.01)
departure = os.path.join(script_config.download_path, departure_file)
download_page = pygetwindow.getWindowsWithTitle("Untitled")[0]
win32gui.PostMessage(download_page._hWnd, win32con.WM_CLOSE, 0, 0)
script_config.tab_reserve(1)
pyautogui.press("enter", interval=0.01)
...
history_forecast_file = f"departure_all_{script_config.download_id}.XML"
pyautogui.hotkey("ctrl", "j", interval=.01)
time.sleep(.25)
pyautogui.press("tab", presses=6, interval=.01)
pyautogui.press("space", interval=.01)
history_forecast = os.path.join(script_config.download_path, history_forecast_file)
download_page = pygetwindow.getWindowsWithTitle("Untitled")[0]
win32gui.PostMessage(download_page._hWnd, win32con.WM_CLOSE, 0, 0)
...
history_forecast_AVC_file = f"departure_all_{script_config.download_id}.XML"

# documents_path = os.environ.get("USERPROFILE").__add__(r"\Documents")

# ori_excel_file = "Briefing.xlsx"
# path_ori_excel = os.path.join(documents_path, ori_excel_file)

# path_td_excel = os.path.join(documents_path, f"{script_config.td_dot_dd_mm_yy}.xlsx")
...
# path_history_forecast = os.path.join(script_config.download_path, history_forecast)
...
# path_history_forecast_AVC = os.path.join(script_config.download_path, history_forecast_AVC)
